...
class BookingRepository {
  ...
  Future<List<BookingTypeEntitiesResponse>> fetchBookingEntities(
      int bookingTypeId) async {
    final response = await _dioClient.get('/bookingsEntity/$bookingTypeId');
    if (response.statusCode == 200) {
      final List<dynamic> data = response.data;
      return data
          .map((json) => BookingTypeEntitiesResponse.fromJson(json))
          .toList();
    } else {
      throw Exception('Ошибка сервера: ${response.statusCode}');
    }
  ...
  Future<BookingsList> fetchBookings(int bookingEntityId) async {
    try {
      final response = await _dioClient.get(
          '/bookingsEntity/$bookingEntityId/bookings',
      ...
      if (e is DioException) {
        throw Exception(
            'Ошибка сети: ${e.message}, Response: ${e.response?.data}');
      }
